...
int check_bit(tree *hft,unsigned char comp,int index)
{
	while(index>=0)
	{
		...
		{
		//	printf("1");
			if(hft != NULL)hft = hft -> right;
		}
		else
		{
		//	printf("0");
			if(hft != NULL)hft = hft -> left;
		}
		index--;
	}
	if(hft == NULL) return 1;
	else return 0;
}
void print_compress(tree *hft, unsigned char comp)
{
	...
	//mask = ~mask;
	aux = index;
	while(!(check_bit(hft,comp,index))) index++;
	
	index--;
	/////////////////////////////// nesse ponto eu tenho exatamente quantos bits eu devo colocar em determinada
	////////////////////////////// variavel
		while(index>=0)
		{
			if(is_bit_i_set(comp,index))
...
void compress(tree *hft, hash_table *ht,char filename[])
{
	int index;
	unsigned int key;
	char value;
	...
	while(!feof(file_in))
	{
		...
		if(key !=255 && key !=10)print_compress(hft,ht->table[key]->freq);
	}
	fclose(file_in);
	printf("\n");
}
...
int main()
{
  ...
  //
  hash2= create_hash_table();
  treetohash(hufftree,hash2,0);
  //
  //print_hash(hash);
  //
  compress(hufftree,hash2,filename);
  //print_compress(hufftree,hash->table[0]->elem);
  return 0;
}
